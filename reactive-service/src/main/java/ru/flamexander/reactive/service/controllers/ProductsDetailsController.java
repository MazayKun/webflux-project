...
public class ProductsDetailsController {
    ...
    public CorePublisher<DetailedProductDto> getAllDetailedProducts() {
        ...
        return combineProductWithDetails(
    ...
    private Flux<DetailedProductDto> combineProductWithDetails(Flux<Product> productDtos, Flux<ProductDetailsDto> detailsDtos) {
        Flux<Map<Long, ProductDetailsDto>> detailsMap = detailsDtos
                .collectMap(ProductDetailsDto::getId)
                .cache()
                .repeat();

        return productDtos.zipWith(
                detailsMap,
                (product, detailsMapArg) -> {
                    ProductDetailsDto details = detailsMapArg.get(product.getId());
                    if(isNull(details)) {
